"""
Servicios para Geology & Geophysics.

Reutiliza las tablas compartidas (app_scripts, app_applications, app_documents)
filtrando por module=MODULE_GEOLOGY_GEOPHYSICS.
Las rutinas tienen su propia tabla (routines) con module="geology_geophysics".
"""
from uuid import UUID
from sqlalchemy.orm import Session

from modules.data_quality.models import MODULE_GEOLOGY_GEOPHYSICS
from modules.data_quality.repository import (
    ...
    get_all_documents, create_document, get_document_by_id,
    get_document_by_file_id, delete_document,
)
...
# Re-export servicio de rutinas directamente
from modules.routines.service import list_routines, get_routine, get_allowed_datasources  # noqa: F401
...
def list_scripts(db: Session) -> list[DQScriptResponse]:
    return [_script_to_response(s) for s in get_all_scripts(db, module=MODULE_GEOLOGY_GEOPHYSICS)]


def get_script(db: Session, id: UUID):
    return get_script_by_id(db, id, module=MODULE_GEOLOGY_GEOPHYSICS)


def create_script_svc(db: Session, body: DQScriptCreate) -> DQScriptResponse:
    s = create_script(
        db, module=MODULE_GEOLOGY_GEOPHYSICS,
...
def update_script_svc(db: Session, id: UUID, content: str) -> DQScriptResponse | None:
    s = get_script_by_id(db, id, module=MODULE_GEOLOGY_GEOPHYSICS)
    if not s:
        return None
...
def list_applications(db: Session) -> list[DQApplicationResponse]:
    return [_app_to_response(a) for a in get_all_applications(db, module=MODULE_GEOLOGY_GEOPHYSICS)]
...
def create_application_svc(db: Session, body: DQApplicationCreate) -> DQApplicationResponse:
    a = create_application(
        db, module=MODULE_GEOLOGY_GEOPHYSICS,
        name=body.name, description=body.description,
        url=body.url, category=body.category,
    )
    return _app_to_response(a)


def delete_application_svc(db: Session, id: UUID) -> bool:
    a = get_application_by_id(db, id, module=MODULE_GEOLOGY_GEOPHYSICS)
    if not a:
        return False
...
def list_documents(db: Session) -> list[DQDocumentResponse]:
    return [_doc_to_response(d) for d in get_all_documents(db, module=MODULE_GEOLOGY_GEOPHYSICS)]
...
def create_document_svc(db: Session, body: DQDocumentCreate) -> DQDocumentResponse:
    d = create_document(
        ...
        content=body.content, url=body.url, file_id=body.file_id,
...
def delete_document_svc(db: Session, id: UUID) -> tuple[bool, UUID | None]:
    ...
    d = get_document_by_id(db, id, module=MODULE_GEOLOGY_GEOPHYSICS)
    if not d:
        return False, None
    fid = d.file_id
...
def upload_file(content: bytes, file_name: str, mime_type: str) -> dict:
    ...
    return {
        ...
        "mimeType": mime_type, "fileSize": len(content),
    }
...
def get_preview_for_file(file_id: str, db: Session) -> dict | None:
    """Busca doc con file_id en módulo GYG, lee archivo, genera preview."""
    try:
        doc = get_document_by_file_id(db, UUID(file_id), module=MODULE_GEOLOGY_GEOPHYSICS)
