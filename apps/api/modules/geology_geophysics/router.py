"""
Endpoints para Geology & Geophysics.

Prefijo: /api/v1/gyg

Expone:
  - Rutinas operacionales (catálogo de scripts ejecutables)
  - Scripts de código (Python, Bash, SQL) del módulo
  - Aplicaciones externas del módulo
  - Documentación (markdown, links, archivos) del módulo
...
from modules.geology_geophysics.service import (
    # Rutinas
    list_routines, get_routine, get_allowed_datasources,
    # Scripts
...
router = APIRouter(prefix="/gyg", tags=["geology-geophysics"])
...
@router.get("/routines/{id_or_slug}", response_model=RoutineResponse)
...
@router.get("/routines/{id_or_slug}/datasources")
def get_routine_datasources(
    ...
    """Lista datasources permitidos para ejecutar esta rutina en el módulo."""
