...
export default function DataQualityTabs({
  ...
  const [scripts, setScripts] = useState<DQScript[]>(initialScripts);
  const [applications, setApplications] = useState<DQApplication[]>(initialApplications);
  const [documents, setDocuments] = useState<DQDocument[]>(initialDocuments);
  ...
  const [panelScript, setPanelScript] = useState<DQScript | null>(null);
  const [panelEditMode, setPanelEditMode] = useState(false);

  // Side panel state — documents
  const [panelDoc, setPanelDoc] = useState<DQDocument | null>(null);
  ...
  const handleAddScript = (script: DQScript) => {
    setScripts((prev) => [...prev, script]);
    setShowNewScript(false);
  };

  const handleAddApp = (app: DQApplication) => {
    setApplications((prev) => [...prev, app]);
    setShowNewApp(false);
  };

  const handleAddDoc = (doc: DQDocument) => {
...
function ScriptListView({
  ...
  scripts: DQScript[];
  onView: (s: DQScript) => void;
  onEdit: (s: DQScript) => void;
...
function DocListView({
  ...
  const [openDoc, setOpenDoc] = useState<DQDocument | null>(null);
...
function DocMarkdownModal({ doc, onClose }: { doc: DQDocument; onClose: () => void }) {
...
function NewDocModal({
  ...
  return (
    <ModalWrapper title="Nuevo Documento" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Título *">
          <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Guía de calidad de datos" className={inputCls(false)} />
        </Field>
        <Field label="Descripción">
